...
@RN2377 introduces the metaphors "sieve" and a "hub" to describe how sociologists conceive of the functions of higher education. Sociology's stratification and schooling tradition conceives of higher education as a "sieve" -- an instrument that strains and separates -- that allocates people to occupational and social positions. The sieve metaphor builds on "Max Weber's core insight that education has a dual character" [@RN2377, p. 129], on one hand, serving social mobility by redistributing access to privileged positions and, on the other, serving social reproduction by granting privileged families disproportionate access to universities that confer privileged positions. 

[CUT THIS PARAGRAPH AND JUST SKIP TO RELIANCE ON PATRONAGE FROM ELITE ACTORS] Building on the sociology of organizational behavior, @RN2377 conceives of higher education as a "hub" that connects, pollinates, and legitimizes "prominent institutional sectors of modern societies" (e.g., professions, the philanthropic sector, nation-state, larger economy).  The paradox, writes @RN2377 is that as a hub, higher education is "simultaneously a powerful and fragile social institution. On the one hand, higher education [is powerful because it] connects and reciprocally blesses various forms of privilege. Elite groups (e.g., high-status families, professions, politicians, scientists, intellectuals) and those aspiring to be a part of them use higher education to certify their legitimacy" (p. 137). On the other hand, higher education is fragile because it depends on "patronage as a primary source of revenue."

Building on @RN2377, @RN4517 introduces three historical configurations that describe the relationship between higher education and society. The "association configuration" -- which defined higher education through the 19th century, but remains important particularly for private colleges -- views colleges as voluntary associations that depended on patronage from other associations (e.g., religious congregations, local business interests, professions). Second, the "national service" configuration -- most associated with the decades following World War II -- views higher education as an engine of nation building and economic development and thus worthy of investment from federal and state givernments. Third, "market" configuration -- which has emerged since the 1980s -- is associated with declining public investment, and competition between colleges for "students, patrons, and prestige" (p. 121).

These metaphors -- which describe higher education as simultaneously serving social mobility and social reproduction and depending on patronage from the government, from volunary associations, and private households -- provide a helpful lense from which to view the recruiting behavior of colleges and universities.  Selective private colleges and universities operate substantially under the association configuration and primarily serve the social reproduction of families and institutions they depend on for patronage. Public universities were founded from the "national service" configuration to serve social mobility and economic development, but declines in public funding compel public universities to find new patrons. 

This paper analyzes the recruiting behavior of selective private colleges and universities to that of public research universities....
...
means of nation building and economic development and 

which defined higher education through the 19th century 

<!-- 
\cite{RN4517} describes the transformation from a ``national service'' political economy to a ``market'' political economy. Under the national service arrangement the state views higher education as a primary means of nation building and economic development \citep{RN4517}. Thus, the national service arrangement is associated with federal and state investments in higher education. The decades following World War II are most associated with this arrangement. At the federal level, the GI Bill in 1944, the National Defense Education Act of 1958, and the Higher Education Act of 1965 collectively increased federal investment in university research and created the federal financial aid system. State governments dramatically increased appropriations to public universities, with the California Master Plan serving as a model for other states. This federal and state funded expansion institutionalized the role of higher education in determining the allocation of individuals to occupations. In turn, higher education became the primary mechanism of social reproduction and a primary means of social mobility. This arrangement benefited universities directly, through increased government funding, and indirectly, in that ``they became the purveyors of the very credentials increasingly required for prosperous lives'' \citep[p.~128]{RN4517}.
-->



``patronage as a primary revenue source'' \citep[p.~138]{RN2377}. 


The paradox, writes @RN2377 is that as 


As a hub, @RN2377 writes that the paradox of higher 

Building on the sociology of organizational behavior, @RN2377 conceives of higher education as a "hub" that connects, pollinates, and legitimizes "prominent institutional sectors of modern societies" (e.g., professions, philanthropic sector, : the labor market and the larger economy, the professions and the sciences, the philanthropic sector, the family, and the nation-state" (p. 135). 

core functions of higher education.
@RN2377 synthesizes how sociology conceives of the functions of higher education, introducing  functions of higher education by introducing

@RN2377 synthesizes how sociology conceives the functions of higher education by introducing the metaphors "sieve," "incubator," "temple," and "hub." As a sieve - an instrument that strains and separates - higher education allocates people to occupational and social positions. The sieve metaphor builds on "Max Weber's core insight that education has a dual character" [@RN2377, p. 129], on one hand, serving social mobility by redistributing access to privileged positions and, on the other, serving social reproduction by granting privileged families disproportionate access to universities that confer privileged positions. 
...
knitr::kable(
  df %>% select(-name,-closeness), caption = 'The 20 most central private high schools with respect to visits from private colleges and universities') %>% 
...
knitr::kable(
  df %>% select(-name,-closeness), caption = 'The 20 most central private high schools with respect to visits from public research universities') %>% 
  landscape(margin = NULL) %>%
  kable_styling(latex_options = 'scale_down')
```
...
knitr::kable(df, caption = 'characteristics of high schools by degree band, public research universities') %>% 
  landscape(margin = NULL) %>%
  kable_styling(latex_options = 'scale_down')
```
...
```{r plot-1mode-privu, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-privu"}
...
plot_characteristic <- function(df, type, characteristic) {
  sub_df <- df %>% select(univ_name, characteristic) %>%
    gather(key, value, characteristic) %>%
    mutate(value = parse_number(value))

  # colors <- sub_df$color[order(sub_df$University)]
  univ_order <- (df %>% arrange(desc(cluster), univ_name))$univ_name
  #univ_order <- (df %>% arrange(desc(Cluster), Type, as.numeric(Rank), University))$University
...
for (i in 1:length(type_vec)) {
  ...
  b <- plot_characteristic(ego_df, 'Religion', c('catholic', 'christian', 'nonsectarian', 'other'))
...
```{r plot-1mode-pubu, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-pubu"}
knitr::include_graphics('assets/figures/plot_1mode_pubu.pdf')
```

```{r plot-1mode-pubu-outst, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-pubu out-of-state visits only", eval = FALSE}
...
```{r ego-network-figure-pubu, fig.align = 'center', echo = FALSE, fig.cap = "characteristics of private high schools visited by public research universities"}
...
```{r plot-1mode-all, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-all", include= FALSE, eval = FALSE}
knitr::include_graphics('assets/figures/plot_1mode_all.pdf')
```

```{r plot-1mode-all-pubu-outst, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-all, out-of-state visits only for public universities"}
...
```{r ego-network-figure-all-pubu-outst, fig.align = 'center', echo = FALSE, fig.cap = "characteristics of private high schools visited by public and private colleges and universities, out-of-state visits only for public universities"}
