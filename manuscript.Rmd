...
df %>% glimpse()
df %>% select(ID,University,Cluster,total) %>% 
  arrange(Cluster,University) %>% print(n=30)
```
<!--

   ID     University       Cluster total

 1 100751 U of Alabama     1         985

16 126614 CU Boulder       4         379
17 218663 U of S.Carolina  4         486

 2 186380 Rutgers          2         303
 3 106397 U of Arkansas    2         225
 4 201885 U of Cincinnati  2         283
 5 139959 U of Georgia     2         302
 6 155317 U of Kansas      2         235
 7 215293 U of Pitt        2         261
 8 110635 UC Berkeley      2         208
 9 110680 UC San Diego     2         197
10 166629 UMass Amherst    2         292

11 199193 NCSU             3          23
12 196097 SUNY Stony Brook 3         146
13 110653 UC Irvine        3          60
14 110671 UC Riverside     3          87
15 181464 UNL              3         159
...
Figure \@ref(fig:plot-1mode-pubu) plots the one-mode network of visits by 17 public research universities, with vertex color determined by hierarchical cluster analysis specified to create $k = 4$ clusters. Accompanying Figure \@ref(fig:plot-1mode-pubu) and created from the respective ego networks of each university is Figure \@ref(fig:ego-network-figure-pubu), which shows the characteristics of private high schools visited by these public reearch universities. Figure \@ref(fig:ego-network-figure-pubu) indicates that public research universities tended to visit private high schools with weaker academic reputation than the set of schools visited by private colleges and universities [LATER, SAY GOING AFTER MONEY WITHOUT ACADEMIC REP]. More surprisingly, despite being nonsectarian, most public research universities visited a higher share of private high schools that were Catholic or Chrisitan than those visited by private colleges and universities.`

The University of Alabama is a cluster unto itself. In 2017, the University of Alabama made 985 visits to private high schools. This number was more than double the 486 visits made by the University of South Carolina, which ranked second amongst public research universities in terms of number of private school visits, and substantially more than the 687 visits made by Notre Dame, which ranked first amongst private colleges and universities. With respect to geographic region, more than 50% of visits were in the South. 

[AT SOME POINT SAY THAT THE GOEGRAPHIC REGION IS NOT ALWAYS THE SAME AS REGION THEY ARE LOCATED IN; IN GENERAL SAY A LOT ABOUT GEOGRAPHIC REGION]

having made 985 visits to U.S. private high schools in 2017 In 2017
At 985 visits, the University of Alabama made more 

created from the respective ego networks of each public research university, shows characteristics of private high schools visited our sample of private colleges and universities, specifically, geographic region, religious affiliation, academic reputation, and racial composition.
...
shit to say about comparison

- on avg. private colleges and universities made more visits than publics, except university of Aalabama.
- 

```{r, include = FALSE, eval = FALSE}
df <- readRDS('./assets/tables/table_ego_all.RDS')
df %>% glimpse()
df %>% select(ID,University,Cluster,total) %>% 
  arrange(Cluster,total) %>% print(n=50)
```
# References
...
plot_characteristic <- function(df, type, characteristics) {
  ...
  univ_order <- (df %>% arrange(desc(Cluster), University))$University
...
for (i in 1:length(type_vec)) {
  ...
  b <- plot_characteristic(ego_df, 'Religion', c('Catholic', 'Christian', 'Nonsect', 'Other'))
...
```{r ego-network-figure-all, fig.align = 'center', echo = FALSE, fig.cap = "characteristics of private high schools visited by public and private colleges and universities"}
knitr::include_graphics('assets/figures/ego_network_figure_all.pdf')
