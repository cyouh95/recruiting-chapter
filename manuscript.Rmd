...
Figure \@ref(fig:plot-1mode-privu) plots the one-mode network of visits by private colleges and universities. Here, the number of ties between a pair of universities (e.g., Sewanee and Texas Christian University) equals the number of private high schools visited by both universities. The graph layout places colleges/universities closer together if they have similar network structure [MAYBE SAY WHAT THIS MEANS IF YOU HAVEN'T YET].  Additionally, vertex color is determined by hierarchical cluster analysis specified to create $k = 4$ clusters.  Although the process of placing vertices was separate from the process of determining community cluster, vertices within the same cluster tend to be closer to one another than those of different clusters. Accompanying Figure \@ref(fig:plot-1mode-privu) is Figure \@ref(fig:ego-network-figure-privu), which shows the characteristics -- geographic region, religious affiliation, academic reputation, and racial composition -- of private high schools visited our sample of private colleges and universities.

One cluster consists of Notre Dame, Villanova, Tulane, and Southern Methodist University. [text about this cluster]

One cluster consists of Baylor, Harvey Mudd, Scripps, and Stevens Institute of Technology. Figure \@ref(fig:plot-1mode-privu) indicates that all four institutions are rather peripheral to the overall network. Harvey Mudd, Scripps and -- to a lesser extent -- Stevens made relatively fewer visits to private high schools. By contrast, Baylor visited a fundamentally different set of schools than other private colleges and universities in that they visited a large number of conservative Christian private high schools.

Figure \@ref(fig:ego-network-figure-privu) indicates that, with the exception of Harvey Mudd and Scripps, these four institutions share little in common with respect to the characteristics of high schools they visit. 
...
df %>% select(ID,University,Cluster,total) %>% 
  arrange(Cluster,University)
```




is based on ego graphs of private colleges and universities and 
<!--
 ...
 2 Harvey Mudd             1
 3 Scripps                 1
 4 Stevens Ins. Tech       1
 5 Case Western Res.       2
 6 Macalester              2
 7 Occidental              2
 8 Smith                   2
 9 Swarthmore              2
...
```{r, ego-table-privu, echo = FALSE, include = FALSE, eval = FALSE}
df <- readRDS('./assets/tables/table_ego_privu.RDS')
knitr::kable(
...
```{r, ego-table-pubu, echo = FALSE, include = FALSE, eval = FALSE}
...
\newpage
```{r ego-network-figure-pubu, fig.align = 'center', echo = FALSE, fig.cap = "characteristics of private high schools visited by private colleges and universities"}
...
```{r plot-1mode-pubu-outst, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-pubu out-of-state visits only"}
knitr::include_graphics('assets/figures/plot_1mode_pubu_outst.pdf')
```

```{r plot-1mode-all, fig.align = 'center', echo = FALSE, out.width = "100%", fig.cap = "caption plot-1mode-all"}
